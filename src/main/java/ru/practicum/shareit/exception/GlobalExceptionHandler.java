...
public class GlobalExceptionHandler {
    ...
    public org.springframework.http.ResponseEntity<ApiError> handleConflict(ru.practicum.shareit.exception.ConflictException ex) {
        return org.springframework.http.ResponseEntity.status(org.springframework.http.HttpStatus.CONFLICT).body(new ApiError("Conflict", ex.getMessage()));
    }

    @ExceptionHandler(Exception.class)
    public ResponseEntity<ApiError> handleOther(Exception ex) {
        ex.printStackTrace();
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(new ApiError("InternalError", ex.getMessage()));
    }
}
